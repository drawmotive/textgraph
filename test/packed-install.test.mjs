...
test('npm tarball works outside the private workspace with no development dependencies', async () => {
  ...
  try {
    ...
    const { stdout: fontPackOutput } = await exec(process.execPath, [npm, 'pack', '--ignore-scripts', '--json', '--workspaces=false', '--pack-destination', root], { cwd: path.join(packageRoot, 'language-packs/zh-cn') });
    ...
    await exec(process.execPath, [npm, 'install', '--ignore-scripts', '--no-audit', '--no-fund', path.join(root, fontPack.filename)], { cwd: consumer });
